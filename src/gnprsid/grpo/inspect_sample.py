...
def _load_grpo_rows(grpo_path: Path) -> list[dict[str, Any]]:
    if not grpo_path.exists():
        raise FileNotFoundError(f"Missing GRPO dataset: {grpo_path}")
    return list(iter_jsonl(grpo_path))
...
def inspect_grpo_sample(
    ...
    if grpo_data_path is None:
        default_name = "valid.jsonl" if split == "valid" else "train.jsonl"
    ...
    if sample_id is not None:
        for candidate in rows:
            if str(candidate.get("sample_id")) == str(sample_id):
                row = candidate
                break
        if row is None:
